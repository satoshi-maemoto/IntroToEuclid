define(['jquery',
    ...
    var monitorView = Backbone.View.extend({
        ...
        monitorList: ['color', 'depth', 'fisheye', 'person', 'imu', 'trajectory', 'teleop', 'filtered'],

        monitorPaths: {
            color: { topic: '/camera/color/image_raw (Downsampled 30%)', path: 'http://' + GlobalSettings.system_ip + ':8080/stream?topic=/camera/color/image_raw&quality=30' },
            depth: { topic: '/camera/depth/image_transcoded (Downsampled 10%)', path: 'http://' + GlobalSettings.system_ip + ':8080/stream?topic=/camera/depth/image_transcoded&quality=10' },
            fisheye: { topic: '/camera/fisheye/image_raw (Downsampled 20%)', path: 'http://' + GlobalSettings.system_ip + ':8080/stream?topic=/camera/fisheye/image_raw&quality=20&height=240&width=320' },
            person: { topic: '/camera/person/detection_image (Downsampled 30%)', path: 'http://' + GlobalSettings.system_ip + ':8080/stream?topic=/camera/person/detection_image&quality=30' },
            ...
            filtered: { topic: '/camera/color/filtered (Downsampled 30%)', path: 'http://' +
GlobalSettings.system_ip + ':8080/stream?topic=/camera/color/filtered&quality=30' }
